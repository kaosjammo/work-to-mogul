...
- **Mogul Stories** (hidden-narrative framework — [`docs/mogul-stories.md`](docs/mogul-stories.md)): rare, optional, industry-specific mini visual novels with hidden scoring + outcome bands + rewards. Reusable `MogulStory` shape (`content/mogulStories/`) + generic modal (`ui/mogulStories/`); player-triggered → harness-inert. Reference story: **Angel Investment** (FridgeMind → founds the Startup Combinator business w/ periodic exit jackpots). Variable length (short/standard/major) so new stories can be authored per industry.
