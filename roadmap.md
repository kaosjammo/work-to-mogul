...
- **Mogul Stories** (hidden-narrative framework — [`docs/mogul-stories.md`](docs/mogul-stories.md)): rare, optional, industry-specific mini visual novels with hidden scoring + outcome bands + rewards. Reusable `MogulStory` shape (`content/mogulStories/`) + generic modal (`ui/mogulStories/`); player-triggered → harness-inert. The shared runtime drives **any registered story by id** (`AngelDealState.storyId` → `activeStory()`; modal/view/stage-nav via `getMogulStory`, save-round-tripped) with a **generic negotiation resolution** (owns-industry trigger + `completedCount` rotation + a timed boost on the story's own industry via `boostIndustryId`) — so most business-deal stories are "content + registration" only; only unique rewards need a bespoke resolver. **🏁 7 stories live — all industries covered:** **Angel Investment** (Finance/FridgeMind → founds the Startup Combinator w/ exit jackpots), **The Lease** (Retail/Thorne Plaza, landlord showdown), **The Poach** (Tech, engineer-retention drama), **Gone Viral** (Food, viral-moment crisis), **The Walkout** (Logistics/Pier 9, dock-strike negotiation), **The Inspection** (Energy/Della Voss, regulator compliance drama), **Scrub or Fly** (Space/Meridian-1, launch gamble w/ a **bespoke empire-wide** reward). Variable length (Angel 10 / Launch 9 / Poach 8 / Walkout 8 / Inspection 8 / Lease 7 / Viral 5). Each has a distinct offer icon (💼🏢👔📱🏗️⚛️🚀). Two bespoke rewards exist (Angel→Combinator, Space→empire-wide halo); the rest share the generic negotiation resolution. **PLUS the Romance Arc** (user-directed, 2026-07-02 — the "no dating" constraint was explicitly lifted): a 4-episode Quinn Harlow love story (💘🌹🏝️💍, 34 stages, relationship-gated via `engine/romance.ts`, industry-free) ending in **marriage → a 20-level money-sink** (+1% of income/s upkeep per level, opt-in, persists through prestige, bot-inert). Opportunity chips now ROTATE: stories get right-of-way over the Salvage Signal, which gained a "Not now" 30-min snooze (was an undismissable 60s-re-arm nag).
...
- **Lunch Rush (NEW):** Vampire-Survivors-style Food-industry mini-game — the food truck auto-throws 🌭 at swarming rabid fans, drag-to-move, mid-run pick-1-of-3 level-up upgrades, 3-tier campaign (Lunch/Dinner/Festival) ending in the permanent 🏆 Golden Spatula (+10% Food). Emoji sprites (zero assets). `content/foodFrenzy.ts` + `engine/foodFrenzy.ts` + `ui/shared/FoodFrenzyGame.tsx`. Opportunity chips now have a strict rotation: **story > salvage > rush** (rush shares the salvage chip slot). Harness-inert.
- **Event Cards 2.0 (PLANNED):** [docs/event-cards-2.0-plan.md](docs/event-cards-2.0-plan.md) — consequence beats, staff-gated third options, targeted effects, deterministic chains, Storm crises, gamble pity. Phase 1 is the next highest-feel-per-effort slice when picked up.
- **Automation managers (NEW):** [docs/automation-managers.md](docs/automation-managers.md) — two opt-in managers configured from a menu (Stats → AUTOMATION). **Executive Assistant** auto-reinvests `cash×(1−reserve%)` on a cadence via a strategy (Best ROI / Cheapest / Focus-industry); **Chief of Staff** auto-hires (operators-first), levels (cheapest-first), and Auto-Assigns within a `budgetPct` budget. `engine/automation.ts` + `ui/shared/AutomationModal.tsx`. OFF by default → harness-inert; bounded/deterministic; config persists through prestige + save; online-only.
