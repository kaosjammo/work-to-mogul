...
- **Phase 1 — Flagship Business screen.** (partly done)
  ...
  - ✅ **1b-i — Business screen chrome.** 4 cue boxes (IndustryBonus + Finance/Quantum/Logistics) → ONE borderless themed `IndustrySignatureStrip` (accent left stripe + bonus baseline; per-industry signature value/Dispatch chip on the right); Spend-Cash → `.btn-secondary .btn-sm` chip in a "Businesses" section header; onboarding hint + entry-cost → borderless accent-left-stripe callouts. Verified 375px: full-border boxes in main 7→3 (WorkCard + banner + list), no overflow.
  - ⬜ **1b-ii — remaining:** WorkCard → borderless accent-stripe strip + content-hug Work Shift; IndustryTabs/Banner → 40px pills + borderless banner; TopHUD → single tier, buy-mode → slim Business-tab row, cut `--hud-h` toward ~80px.
