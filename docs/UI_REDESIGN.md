...
- **Phase 3 — structural ✅ / game-world ⬜.** ✅ Slim Prestige hero + accent-stripe talent/milestone rows; ✅ ONE shared `Overlay` shell for EventCard/DailyBonus/Ascension/WelcomeBack (confirms `.btn-lg .btn-block`, dismiss `.btn-ghost`; EventCard options → accent-stripe rows). ⬜ **remaining = FULL game-world:** reactive mascot poses (idle/working/excited) above the fold + `cash_burst` on hero actions; concentrate juice. Keep `prefers-reduced-motion`.
